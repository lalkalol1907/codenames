...
class RoomService(
    ...
    fun createRoom(language: Language, hostName: String): Pair<Room, Player> {
        ...
        metrics.incrementRoomsCreated(language.code, "web")
        return room to host
    }
    ...
    fun bootstrapDiscordRoom(
        ...
        if (existingRoom != null) {
            ...
            if (existingPlayer != null) {
                metrics.incrementDiscordBootstrap("existing")
            ...
            metrics.incrementDiscordBootstrap("join")
        ...
        metrics.incrementRoomsCreated(language.code, "discord")
        metrics.incrementDiscordBootstrap("new")
        return room to host
    }
    ...
    fun randomizeTeams(roomCode: String, hostPlayerId: UUID): Room {
        ...
        if (!RoomTeamRules.canRandomize(activePlayers.size)) {
            throw RoomException("Between 4 and 8 active players required for random teams")
        }
        val assignments = RoomTeamRules.randomRoleAssignments(activePlayers)
    ...
    fun startGame(roomCode: String, hostPlayerId: UUID): Room {
        ...
        val result = roomRepository.startGameAtomically(room.code, hostPlayerId, game) { lockedRoom ->
            validateReadyToStart(lockedRoom)
        }
        metrics.incrementGamesStarted(room.language.code)
        return result
    }
    ...
    private fun validateRoleAssignment(room: Room, playerId: UUID, team: Team?, role: Role) {
        RoomTeamRules.validateRoleAssignment(room.players, playerId, team, role)
    }

    private fun validateReadyToStart(room: Room) {
        RoomTeamRules.validateReadyToStart(room.players)
    }
